...
.equ    cnt0Top     =   0x10    ; slow clock sets all gpio1 flags
...
BufEnd:
LedRSet: .Byte 1				; 0xab = a on cyc b off cyc 
LedGSet: .Byte 1				; 
...
.equ 	leSPin		=	PINB
.equ 	leSDir		=	DDRB
; ADC 1 for SFH309 vs GND with 4k7 pup -> change Analog Select
.equ 	anaNum		=   4         
.equ 	anaPrt		=	PORTB
.equ 	anaPin		=	PINB
.equ 	anaDir		=	DDRB
...
.equ	anaRaw 		= 6			; 1 to store raw data,	0 to evl signal
...
;	PortOut sh0Num, sh0Dir, sh0Prt
	PortOut leSNum, leSDir, leSPrt
    PortOut leRNum, leRDir, leRPrt
    PortOut leGNum, leGDir, leGPrt
...
start_1: ; Normalbetrieb
	...
	ldi		tmp,1
	mov		cnt0,tmp
	ldi		nacCnt, 3 ; immediately alert
...
;*****************
Eval:
; something received, fetch it 0000ccoo
	mov     tmp,inData
	cbi	    myGP,inDataValid
	ldi		nacCnt, 14
; #TODO check if valid 
; act on inData, extract cc and oo
	push    tmp ; to send back
	push	tmp ; des
	andi	tmp,0x03
	mov		inOri,tmp
	pop		tmp
...
Eval_Cmd:
;set led depending on incol
	ldi		tmpL, 0x0F ; always on
	rcall	setLed
Eval_Back:
; Answer back received data
	pop		tmp
	rcall	putch
	ret

SetLed:
; set blnk mycol 2 R 3 G else unknown tmpL (0..F) denotes switch-off, other col always off
	mov		tmp,inCol
	cpi		tmp, 2
	breq	SetLed_R
	cpi		tmp, 3
...
; Led Counter from ledSet
	cbi		myCn0, cn0Led  
; Led switching on/off depends on ledCyc 0..x
	inc     ledCyc
	cpi		ledCyc, 12
	brne	DoLed_1
	clr		ledCyc
