...
C_w:
	...
	cpi	tmp,'t' 	; analog again
	brne C_t
	rcall ADC_Temp
	rjmp Lop4
C_t:

	cpi	tmp,'v' 	; activate out
	brne C_v
	rcall ADC_VCC
	rjmp Lop4
C_v:
...
;
; ADC settings for tiny x5
ADC_Temp:  ; Temperature internal
	ldi		tmp, (0<<REFS2)+(1<<REFS1)+(0<<REFS0)+(1<<ADLAR)+(1<<MUX3)+(1<<MUX2)+(1<<MUX1)+ (1<<MUX0)
	rjmp	ADC_One

ADC_110:  ; Ref 1.1 V
	ldi		tmp, (1<<REFS2)+(1<<REFS1)+(0<<REFS0)+(1<<ADLAR)+(0<<MUX3)+(0<<MUX2)+(1<<MUX1)+ (0<<MUX0)
	rjmp	ADC_One
...
ADC_VCC:  ; for x5, Single conversion mode
; REFS2 REFS1 REFS0 Voltage Reference Selection
;   x	  0     0   VCC used as analog reference, disconnected from PB0 (AREF)
;   x	  0     1   External voltage reference at AREF pin, internal reference turned off
;   0       1     0   Internal 1.1V voltage reference
;   1       1     0   Internal 2.56V Voltage Reference disconnected from PB0
...
; ADATE, ADIF, ADIE
; ADPS2:0: ADC Prescaler Select Bits should be 50..200 khz
; 0 0 0   2
; 0 0 1   2
; 0 1 0   4
; 0 1 1   8
...
; 1 1 0  64
; 1 1 1 128
	ldi		tmp, (1<<ADEN)+(1<<ADSC)+(0<<ADATE)+(1<<ADPS2) + (0<<ADPS1) + (1<<ADPS0)
	out		ADCSRA,tmp
	clr		tmp
	out		ADCSRB,tmp
	ret
